...
const displayProducts = () => {
    ...
    data.forEach((product) => {
        ...
        productItem.innerHTML += `<a href="productDetails.html">
        <div class="product__image">
            <img src="${product.image}" alt="">
        </div>

        <div class="flex flex-row gap-[183px] mt-[20px] product__info">
                   
        <div class="product__description">
            <h3 class="text-[20px] font-bold">${product.name}</h3>
            <p class="text-[16px] font-bold opacity-[50%] mt-[10px]">${product.category}</p>
        </div>
     
        <div class="product__price"><h2 class="text-[24px] font-bold">${product.price} USD</h2></div>
         </div>
        </a>`
        ...
        products.insertAdjacentElement('afterbegin', productItem)
    });
